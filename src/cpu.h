...
extern Address global_current_pc;              // Чистый адрес начала текущей инструкции для ловушек
extern Word current_instruction_word;          // Текущее считанное слово инструкции для проверок АЛУ
...
void set_psw(Word psw);                         //функция распаковки 16-битного слова PSW во флаги и приоритет
...
void do_mtps(void);     // MTPS     [1064DD] NZVC=**** | Запись в регистр состояния PSW из приемника
void do_mfps(void);     // MFPS     [1067DD] NZVC=**0- | Чтение регистра состояния PSW в приемник

//служебные
void do_rti(void);      // RTI      [000002] NZVC=vvvv | Возврат из обработчика прерывания через стек
void do_rtt(void);      // RTT      [000006] NZVC=---- | Возврат из системной отладочной ловушки
...
void do_trap244(void);  // TRAP244  [VEC_244] NZVC=----| Ловушка математических ошибок плавающей точки (FIS/FPU)
void do_ldf(void);      // LDF      [172400]  NZVC=vv00| Загрузка вещественного значения во float-регистр
void do_stf(void);      // STF      [174000]  NZVC=----| Сохранение float-регистра в память/регистр
void do_trap10(void);   // TRAP10   [VEC_10] NZVC=---- | Ловушка Нереализованной команды / Запрещенного опкода по вектору 000010
